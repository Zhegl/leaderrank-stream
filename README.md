...
Итого $O(\textrm{iters} \cdot (\frac{|E|}{\textrm{batch}} + |V|))$, где $\textrm{batch}$ - сколько читаем за раз. ($|E|$ делим, т.к. доступ к ребрам последовательный, а в случае рандомного доступа кеш не меняет асимптотику).
